...
{
	/// <summary>
	/// Canceling a System.Net.Websocket on Mono causes the websocket to abort and sends an unobservedtaskexeption
	/// So this class doesn't cancel the current read/write operation but does send up an OperationCanceledException
	/// Seems related to: https://github.com/dotnet/corefx/issues/29005
	/// </summary>
	public class MqttMonoWebSocketChannel : IMqttChannel
	{
		...
		public MqttMonoWebSocketChannel (MqttClientWebSocketOptions options)
		{
			_options = options ?? throw new ArgumentNullException (nameof (options));
		}

		public MqttMonoWebSocketChannel (WebSocket webSocket, string endpoint)
		{
		...
		{
			using (var ct = cancellationToken.Register (() => throw new OperationCanceledException ())) {
				var response = await _webSocket.ReceiveAsync (new ArraySegment<byte> (buffer, offset, count), CancellationToken.None).ConfigureAwait (false);
		...
		public async Task WriteAsync (byte [] buffer, int offset, int count, CancellationToken cancellationToken)
		{
			...
			try {
				using (var ct = cancellationToken.Register (() => throw new OperationCanceledException ())) {
					await _webSocket.SendAsync (new ArraySegment<byte> (buffer, offset, count), WebSocketMessageType.Binary, true, CancellationToken.None).ConfigureAwait (false);
				}
			} finally {
				_sendLock.Release ();
			}
